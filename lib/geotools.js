...
function _powerup() {
    ...
    var rawCities = fs.readFileSync(path.join(GEO_DIR_DATA, 'cities.txt'));


    var iconv = new Iconv('WINDOWS-1251', 'UTF-8');
    var buffer = iconv.convert(rawCities);
    var cities = buffer.toString('utf8').split('\n');
...
function _normalize(row) {
    return {
        ...
        "cityInfo": gCities[row[GEO_FIELD_REGION]]
    };
};
